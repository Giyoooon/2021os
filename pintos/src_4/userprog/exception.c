...
{ /* True: not-present page, false: writing r/o page. */
  ...
  void* esp = user ? f->esp : curr->prev_esp;

  // Stack Growth
  bool is_user_stack, push_avaliable;
  is_user_stack = (PHYS_BASE - MAX_STACK_SIZE <= fault_addr && fault_addr < PHYS_BASE);

  push_avaliable = (esp <= fault_addr || fault_addr == f->esp - 4 || fault_addr == f->esp - 32);
  if (is_user_stack && push_avaliable) {
    ...
    if (vme_has_entry(curr->vm, fault_page) == false)
      is_stack_growth(curr->vm, fault_page);
  }
  if(!handle_mm_fault(curr->vm, curr->pagedir, fault_page) ) {
