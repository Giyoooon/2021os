...
//bool handle_mm_fault(sturct vm_entry *vme);
...
process_execute (const char *file_name) 
{
  char *fn_copy;
	...
	filename_parser(file_name,only_file_name);
  ...
  strlcpy (fn_copy, file_name, PGSIZE);
	...
	if(filesys_open(only_file_name) == NULL) 
		return -1;

	

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (only_file_name, PRI_DEFAULT, start_process, fn_copy);
...
start_process (void *file_name_)
{
  ...
  success = load (only_file_name, &if_.eip, &if_.esp);
...
process_exit (void)
{
  ...
  /* Destroy the current process's page directory and switch back
	...
	struct list_elem *e;
	//struct mmap_descriptor mm_desc;
	/*
	while(!list_empty(mm_list)){
		e = list_begin(mm_list);
		mm_desc = list_entry(e, struct mmap_descriptor, elem);
		ASSERT(custom_munmap(mm_desc->id) == true);
	}
	*/
...
#ifdef VM
			struct thread* current_threads = thread_current();
			ASSERT(pagedir_get_page(current_threads->pagedir, upage) == NULL);

			if(!vm_install_filesys(current_threads->vm, upage, file, ofs, page_read_bytes, page_zero_bytes, writable)){
				return false;
			}
#endif
...
{
  struct thread *t = thread_current ();
	bool tmp1, tmp2, success;
  /* Verify that there's not already a page at that virtual
     address, then map our page there. */
	tmp1 = (pagedir_get_page(t->pagedir,upage) == NULL);
	tmp2 = pagedir_set_page(t->pagedir, upage, kpage, writable);
	success = tmp1 && tmp2;
#ifdef VM
