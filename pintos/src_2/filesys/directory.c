            ...
            struct inode **inode) 
{
  ...
  ASSERT (name != NULL);
	...
	inode_lock(dir_get_inode(dir_get_inode((struct dir*)dir)));

  if (lookup (dir, name, &e, NULL))
    *inode = inode_open (e.inode_sector);
  else
    *inode = NULL;
	inode_unlock(dir_get_inode(dir_get_inode((struct dir*)dir)));
...
dir_add (struct dir *dir, const char *name, block_sector_t inode_sector)
{
  ...
  ASSERT (name != NULL);
	...
	inode_lock(dir_get_inode(dir_get_inode(dir)));
  ...
  if (lookup (dir, name, NULL, NULL))
	...
	if(!inode_set_parent(inode_get_number(dir_get_inode(dir), inode_sector))
		goto done;
...
bool dir_is_dirty(struct inode* inode){
	struct dir_entry e;
	off_t pos = 0;
